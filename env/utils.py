...
def find_closest_position(obj_position: dict, reachable_positions: list, except_pos: list, boundary: list):
    '''
    ...
    for position in reachable_positions:
        if (round(position['x'], 2), round(position['z'], 2)) in except_pos:
            ...
            # print(except_pos)
            continue
        if (not (boundary[0] <= position['x'] <= boundary[1])) or (not (boundary[2] <= position['z'] <= boundary[3])):
            continue
    ...
    print("for debug utils", closest_pos, boundary)
    return closest_pos
...
def find_parent_receptacle_plus(
    ...
    if parent_receptacles == None:
        ...
        obj_full_state = objects[obj_id_2_index[obj_id]]
        ...
        for _recep_obj_id in parent_receptacles:
            if _recep_obj_id not in disabled_objs_id:
                _recep_obj_full_state = objects[obj_id_2_index[_recep_obj_id]]
...
def compute_face_2_pos_rough_plus(obj_id: str, obj_pos: dict, reachable_positions: list, except_pos: list, boundary = None) -> dict:
    '''
    This function will not directly teleport the agent to the position, 
    alternatively computes the pose and correction of the agent roughly . 
        - objId: the Id of the object
    ...
    '''
    obj_x, obj_y, obj_z = obj_pos['x'], obj_pos['y'], obj_pos['z']
    agent_pos = find_closest_position(obj_pos, reachable_positions, except_pos, boundary)
    agent_x, agent_y, agent_z = agent_pos['x'], agent_pos['y'], agent_pos['z']
    # controller.step(
    #     action='Teleport',
    #     position=position,
