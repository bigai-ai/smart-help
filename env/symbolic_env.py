...
class SymbolicEnv(Env):
    def __init__(self, config):
        ...
        self.observation_space = spaces.Box(low=-360, high=360, shape=(30 + self._agent_num, self.obs_width))
    ...
    # For mcts
    def reset_to_init(self, *, seed=None, options=None, task=None, type=None, env_index=None):
        # 10 remain for test
        i = random.randint(1, 20)
        index = i
        # exclude scenario
    ...
    def get_agent_obs(self, agent_id):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if len(self.agents[i].action_history) > 0:
                agent_obs[i, 6:9] = np.asarray([int(agent.action_history[-1][0]), 0, int(agent.action_history[-1][2]) + 1], dtype=np.float32) # for embedding
            else:
