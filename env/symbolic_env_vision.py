...
class SymbolicEnvVision(Env):
    def __init__(self, config):
        ...
        self.observation_space = spaces.Box(low=0, high=256, shape=(3, 224, 224))
    ...
    def reset(self, *, seed=None, options=None, task=None, type=None, env_index=None):
        # 10 remain for test
        ...
        if self._agent_num > 1:
            self._agent_type[1] = sample_type(type)
            ...
            if self.check_sub_task(subtask, agent_id=agent_id, action=action):
                ...
                if agent_id == 0:
                    ...
                    if self.need_help_goal([subtask]):
                        ...
                        print("Necc Subtask", subtask, "Finish! by ", agent_id, "AgentType:", self._agent_type[1])
                        reward += 20
                    else:
                        reward -= 10
                if agent_id == 1:
        ...
        if action_name in ['PickUp', 'Put', 'Open', 'Close', 'ToggleOn', 'ToggleOff', 'Slice']:
            ...
            if action_name == "Open" and self._agent_type[agent_id][2] < 0.5:
                ...
                if self.metadata(agent_id=agent_id)['objects'][obj_index]['position']['y'] > (self._agent_type[agent_id][0] + 0.5):
                    # print("for debug env", "The object is too high!!!")
                    return False
                # mass check
                if self.metadata(agent_id)['objects'][obj_index]['mass'] > self._agent_type[agent_id][1]:
                    # print("for debug env", "The object is too heavy!!!")
                    return False
            # elif action_name in ['ToggleOn', 'ToggleOff'] and self._agent_type[
            #     agent_id].value == AgentType.AGENT_WITH_TOGGLE_ISSUES.value:
            #     return False
            obj_id = self.find_id(self.object_index2name[action_tar])
            if obj_id is False:
                return False
    ...
    def get_obs(self, agent_id):
        image_origin = self.controller.last_event.cv2img
        # 将输入数组转换为PIL图像
        image = Image.fromarray(image_origin)

        # 调整图像大小为224x224，保持纵横比不变
        resized_image = image.resize((224, 224), Image.ANTIALIAS)

        # 创建一个新的224x224的图像
        output_array = np.zeros((3, 224, 224))

        # 将调整后的图像复制到新的数组中
        output_array[:, :resized_image.height, :resized_image.width] = np.array(resized_image).transpose((2, 0, 1))
        return output_array
    ...
    def step(self, goal):
        ...
        if self.step_count >= 30:
            ...
            # punishment 
            rew_accumulate -= 20
        return obs, rew_accumulate, terminated, truncated, info
    ...
    def execuate_goal(self, goal, agent_id=0):
        ...
        if goal[0] == "Wait":
            ...
            self.update_dict()
            if event is False:
                reward_accumulate -= 0.5
            # reward from goal
            reward, _ = self._get_reward_and_done(agent_id, action=action_input)
    ...
    def need_help_goal(self, goal_list, agent_id=0):
        # I edited this because it didn't work in env with only 1 agent -- Siwen
        if self._agent_num == 1:
             return False
