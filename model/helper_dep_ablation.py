...
class HelperModelDepAbla(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        super(HelperModelDepAbla, self).__init__(obs_space, action_space, num_outputs, model_config, name)
        ...
        self.model = nn.Sequential(
            nn.Linear(128 * 2 + num_outputs + 128, num_outputs),
            nn.ReLU(),
        ...
        self.value_model = nn.Sequential(
            nn.Linear(128 * 2 + num_outputs + 128, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
    ...
    def forward(self, input_dict: Dict[str, TensorType], state: List[TensorType], seq_lens: TensorType) -> (TensorType, List[TensorType]):
        ...
        if self.batch_size != 1:
            observation_embedding = torch.cat([obs_flat, subtask_embedding, tar_1_embedding, tar_2_embedding], dim=1)
